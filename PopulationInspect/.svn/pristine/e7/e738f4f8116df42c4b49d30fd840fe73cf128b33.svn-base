...
function commchart(datas) {
	//console.log(datas);
	var chart_data_comm = [];
	chart_data_comm.push({
		value: datas.indebuilding,
		name: '独立建筑'
	}, {
		value: datas.building_number - datas.indebuilding,
		name: '小区建筑'
	});
...
function commchart2(datas) {
	...
	var chart_data_comm = [];
	chart_data_comm.push({
		value: datas.indebuliding_number,
		name: '独立建筑'
	}, {
		value: datas.building_number - datas.indebuliding_number,
		name: '小区建筑'
	});
...
function roomchart(datas) {
	...
	for (var i = 0; i < datas.room_classify_info.length; i++) {
		if (datas.room_classify_info[i].roomclass == 01) {
			chart_data_room.push({
				value: datas.room_classify_info[i].num,
				name: '住宅'
			});
...
function peoplechart(datas) {
	...
	for (var i = 0; i < datas.person_classify_info.length; i++) {
		if (datas.person_classify_info[i].why == 01) {
			chart_data_people.push({
				value: datas.person_classify_info[i].num,
				name: '自住'
			});
...
function getroom(builId, unit_num, floor_num) {
	$.ajax({
		...
		success: function(data) {
			// console.log(data);
			window.localStorage.setItem('room-info', data);
			var datas = eval('(' + data + ')');
			var roombox = $('#roombox');
			var q = '';
			roombox.html('');
